...
export default {
  ...
  methods: {
    ...
    async update() {
      ...
      let driver = await this.driverStore.update(id, this.formData);
  ...
  mounted() {
    const id = this.$route.params.id;
    this.driverStore.show(id).then(() => {
      const driver = this.driverStore.drivers;

      this.formData.name = driver.name;
    });
  },
};
</script>
